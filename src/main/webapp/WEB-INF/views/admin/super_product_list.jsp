...
<main>
	...
	<div class = "type">
	<%if(type.equals("")){ %>
	<a href = "super_product_list" class = "ctgr active">전체</a>
	<%}else{ %>
	<a href = "super_product_list" class = "ctgr">전체</a>
	<%} %>
		...
		%>
			<a href = "super_product_list?type=<%=t.getName()%>" class = "ctgr<%=active%>"><%=t.getName() %></a>
		<%} %>
	</div>
	...
	for(int i = 0;i<productList.size();i++){
		...
		String lnk = "super_product_view?piid="+pi.getIdx();
	%>
		...
		<%} %>
		<p>
		<%if(pi.getIsview().equals("y")){ %> 판매중
		<%}else{ %> 판매중지 <%} %>
		</p>
		<p>
		<%if(pi.getStock()<=0){ %>품절
		<%}else{ %> 재고량: <%=pi.getStock() %><%} %>
		</p>
	</li>
...
<% }%>
	...
	<a href = "super_product_register" class = "ctgr">신규등록</a>

	<div class="paging">
	<%
	
	if(cpage!=1){
		out.println("<a href='super_product_list?page=1"+tmp+"'>처음</a>");
	}else{ 
		out.println("<p>처음</p>");
	}
	
	if(cpage>1){
		out.println("<a href='super_product_list?page="+(cpage-1)+tmp+"'>이전</a>");
	}else{ 
		out.println("<p>이전</p>");
	}
	...
	for(int i = spage; i<=pcnt && i<= spage+bsize-1; i++){
		if(cpage == i){
			...
			out.println("<a href = 'super_product_list?page="+i+tmp+"'>"+i+"</a>");
		}
	}
	
	if(cpage<pcnt){
		out.println("<a href='super_product_list?page="+(cpage+1)+tmp+"'>다음</a>");
	}else{ 
		out.println("<p>다음</p>");
	}
	
	if(cpage!=pcnt){
		out.println("<a href='super_product_list?page="+pcnt+tmp+"'>마지막</a>");
	}else{ 
		out.println("<p>마지막</p>");
	}
	%>
